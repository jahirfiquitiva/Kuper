...
class KuperAdapter(private val context:WeakReference<Context>,
    ...
    override fun onBindHeaderViewHolder(holder:SectionedViewHolder?, section:Int,
        ...
        context.get()?.let {
            if (holder is SectionedHeaderViewHolder) {
                when (section) {
                    0 -> {
                        holder.setTitle("${it.getString(R.string.zooper_widget)} ${it.getString(
                                R.string.templates)}")
                        holder.icon.gone()
                    }
                    1 -> {
                    ...
                    2 -> {
                        holder.setTitle("${it.getString(R.string.kwgt)} ${it.getString(
                                R.string.templates)}")
                        holder.icon.gone()
                    }
                    3 -> {
                        holder.setTitle("${it.getString(R.string.klwp)} ${it.getString(
                                R.string.templates)}")
                        holder.icon.gone()
                    }
                }
